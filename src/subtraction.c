...
fraction32 f32sub (fraction32 x, fraction32 y)
{
	...
	return (
		(uint32_t)((xa * yb) - (ya * xb)
	) << (sizeof(fraction32)*4))
	+ (uint32_t)(xb*yb);
}

fraction16 f16sub (fraction16 x, fraction16 y)
{
	...
	return (
		(uint16_t)((xa * yb) - (ya * xb)
	) << (sizeof(fraction16)*4)) 
	+ (uint16_t)(xb*yb);
}

fraction64 f64sub (fraction64 x, fraction64 y)
{
